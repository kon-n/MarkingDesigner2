...
本リポジトリにおける仕様の正本はこのファイル（ルートの `specification.md`）です。
...
#### 1.3.1 P / C / D / Q / U の動作ルール（確定）

- `P` コマンドは、指定ジョブの**開始点（StartX / StartY）を読み出し**、後続の `C` / `D` / `Q` / `U` の開始位置基準とする。
- `C` コマンドは、指定ジョブの**文字列以外のパラメータ**を、直前 `P` で指定された位置系条件で適用する。  
  そのため、文字回転・円弧・文字列回転など、文字列以外のレイアウト系パラメータの影響を受ける。
- `C` コマンドは「前回刻印内容の反復刻印」とし、**インクリメント更新・カレンダー更新は行わない**。  
  前回刻印した内容をそのまま打刻する。
- `D` / `Q` / `U` コマンドも `P` の開始位置基準に従って配置される。
